...
# Toxicity adjusted
m02_modFrame <- data.frame(Variable = rownames(summary(mod02)$coef),
                             Coefficient = summary(mod02)$coef[, 1],
                             SE = summary(mod02)$coef[, 2],
                             Outcome = "Toxicity",
                          Data = "Raw Data",
...
m04_modFrame <- data.frame(Variable = rownames(summary(mod04)$coef),
                            Coefficient = summary(mod04)$coef[, 1],
                            SE = summary(mod04)$coef[, 2],
                            Outcome = "Comment length",
                            Data = "Raw Data",
...
m11_modFrame <- data.frame(Variable = rownames(summary(mod11)$coef),
                           ...
                           Model = "Toxicity simple")[-1,] # [-1,] removes itercept, delete to include
# Toxicity adjusted
m12_modFrame <- data.frame(Variable = rownames(summary(mod12)$coef),
                           Coefficient = summary(mod12)$coef[, 1],
                           SE = summary(mod12)$coef[, 2],
                           Outcome = "Toxicity",
                           Data = "Disagreement",
                           Model = "Toxicity adjusted")[-1,] 
# Comment length simple
...
# Comment length adjusted
m14_modFrame <- data.frame(Variable = rownames(summary(mod14)$coef),
                           Coefficient = summary(mod14)$coef[, 1],
                           SE = summary(mod14)$coef[, 2],
                           Outcome = "Comment length",
                           Data = "Disagreement",
                           Model = "Comment length adjusted")[-1,] 
# Attitude distance
...
m21_modFrame <- data.frame(Variable = rownames(summary(mod21)$coef),
                           ...
                           Model = "Toxicity simple")[-1,] # [-1,] removes itercept, delete to include
# Toxicity adjusted
m22_modFrame <- data.frame(Variable = rownames(summary(mod22)$coef),
                           Coefficient = summary(mod22)$coef[, 1],
                           SE = summary(mod22)$coef[, 2],
                           Outcome = "Toxicity",
                           Data = "Manipulation",
                           Model = "Toxicity adjusted")[-1,] 
# Comment length simple
...
# Comment length adjusted
m24_modFrame <- data.frame(Variable = rownames(summary(mod24)$coef),
                           Coefficient = summary(mod24)$coef[, 1],
                           SE = summary(mod24)$coef[, 2],
                           Outcome = "Comment length",
                           Data = "Manipulation",
                           Model = "Comment length adjusted")[-1,] 
# Attitude distance
...
m31_modFrame <- data.frame(Variable = rownames(summary(mod31)$coef),
                           ...
                           Model = "Toxicity simple")[-1,] # [-1,] removes itercept, delete to include
# Toxicity adjusted
m32_modFrame <- data.frame(Variable = rownames(summary(mod32)$coef),
                           Coefficient = summary(mod32)$coef[, 1],
                           SE = summary(mod32)$coef[, 2],
                           Outcome = "Toxicity",
                           Data = "Disagreement + Manipulation",
                           Model = "Toxicity adjusted")[-1,] 
# Comment length simple
...
# Comment length adjusted
m34_modFrame <- data.frame(Variable = rownames(summary(mod34)$coef),
                           Coefficient = summary(mod34)$coef[, 1],
                           SE = summary(mod34)$coef[, 2],
                           Outcome = "Comment length",
                           Data = "Disagreement + Manipulation",
                           Model = "Comment length adjusted")[-1,] 
# Attitude distance
...
m41_modFrame <- data.frame(Variable = rownames(summary(mod41)$coef),
                           ...
                           Model = "Toxicity simple")[-1,] # [-1,] removes itercept, delete to include
# Toxicity adjusted
m42_modFrame <- data.frame(Variable = rownames(summary(mod42)$coef),
                           Coefficient = summary(mod42)$coef[, 1],
                           SE = summary(mod42)$coef[, 2],
                           Outcome = "Toxicity",
                           Data = "Answering Time PAP",
                           Model = "Toxicity adjusted")[-1,] 
# Comment length simple
...
# Comment length adjusted
m44_modFrame <- data.frame(Variable = rownames(summary(mod44)$coef),
                           Coefficient = summary(mod44)$coef[, 1],
                           SE = summary(mod44)$coef[, 2],
                           Outcome = "Comment length",
                           Data = "Answering Time PAP",
                           Model = "Comment length adjusted")[-1,] 
# Attitude distance
...
allModelFrame <- data.frame(rbind(m01_modFrame, m02_modFrame,m03_modFrame,m04_modFrame,
                                  m11_modFrame, m12_modFrame,m13_modFrame,m14_modFrame,
                                  m21_modFrame, m22_modFrame,m23_modFrame,m24_modFrame,
                                  m31_modFrame, m32_modFrame,m33_modFrame,m34_modFrame,
                                  m41_modFrame, m42_modFrame,m43_modFrame,m44_modFrame))
...
cp1 <- ggplot(allModelFrame, aes(x = Variable, colour = Data)) + 
  facet_grid( ~ Model)+
  ...
  scale_x_discrete(labels = c("time_online" = "Time spent online",
                              ...
                              "post_directionb" = "Post direction (issue important)",
